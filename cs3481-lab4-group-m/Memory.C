...
uint64_t Memory::getLong(int32_t address, bool & imem_error)
{
   ...
   if (address < 0 || address >= MEMSIZE) //use buildlong() from Tools
   {
      imem_error = true;
      return 0;
   }
   ...
   imem_error = false;
   uint64_t word = Tools::buildLong(bytes);
   return word;
}
...
uint8_t Memory::getByte(int32_t address, bool & imem_error)
{
   ...
   imem_error = false;
   uint64_t word = getLong(address & ~0x7, imem_error);
   uint8_t byteNum = address & 0x7;
   uint8_t byte = Tools::getByte(word, byteNum);
   return byte;
}
...
void Memory::putLong(uint64_t value, int32_t address, bool & imem_error)
{
   ...
   for (int i = 0; i < 8; i++)
   {
      mem[address + i] = (uint8_t)(value >> (i * 8));
   }
