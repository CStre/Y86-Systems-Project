...
{
	uint64_t fir = (uint64_t)bytes[0];
	uint64_t sec = (uint64_t)bytes[1] << 8;
	uint64_t thir = (uint64_t)bytes[2] << 16;
	uint64_t four = (uint64_t)bytes[3] << 24;
	uint64_t five = (uint64_t)bytes[4] << 32;
	uint64_t six = (uint64_t)bytes[5] << 40;
	uint64_t sev = (uint64_t)bytes[6] << 48;
	uint64_t eigh = (uint64_t)bytes[7] << 56;
	uint64_t newByte = (fir | sec | thir | four | five | six | sev | eigh)| 0x0000000000000000;
	return newByte;
}
/** 
...
{
	uint64_t multi = 0x00000000000000ff;
	multi = multi << (byteNum * 2);
	multi = multi << (byteNum * 2);
	multi = multi << (byteNum * 2);
	multi = multi << (byteNum * 2);
	uint64_t ans = multi & source;
	ans = ans >> (byteNum * 8);
	return ans; 
}
...
{
	if(low < 0 || high > 63 || high < low || high < 0 || low > 63)
	{
		return source;
	}
	else
...
{
	//uint64_t ans = source;
	if (srclow < 0 || srclow >= 64 || dstlow < 0 || dstlow >= 64 || length < 1 || length + dstlow > 64)
	{
		return dest;
	}
	else
	{
		uint64_t temp1= getBits(source, srclow, srclow + length - 1);
		uint64_t temp2 = clearBits(dest, dstlow, dstlow + length - 1);
		temp1 = temp1 << dstlow;
		return temp1 | temp2;
	}
}
...
{
	uint64_t sign1 = sign(op2 - op1);
	uint64_t sign2 = sign(op1);
	return ((sign1 == sign2));
}
